  ...
  return (
    <Container>
      ...
      <menu className="block pt-8 pb-16 space-y-8 lg:hidden">
        {experience.map((item, index) => (
          <li key={index} className={`flex items-center gap-4 ${index % 2 === 0 ? "flex-row" : "flex-row-reverse"}`}>
            <div className="relative p-4 shadow-2xl rounded-2xl bg-dark shadow-light/30">
      ...
      <menu className="hidden grid-cols-2 px-8 pb-28 pt-28 gap-x-12 gap-y-60 lg:grid">
        {experience.map((item, index) => (
          <li key={index} className={`flex items-center justify-center gap-4 ${index % 2 === 0 ? "-mt-96" : ""}`}>
